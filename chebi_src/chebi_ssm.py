...
def map_to_chebi(text, name_to_id, synonym_to_id):
    ...
    if text.endswith("s") and text[:-1] in chebi_cache["fuzzyratio"]:
        ...
        drugs = process.extract(text, name_to_id.keys()) #, scorer=fuzz.token_sort_ratio)
        #print("best names of ", text, ":", drugs)
        if drugs[0][1] == 100:
            drugs = [drugs[0]]
        if drugs[0][1] < 70:
            drug_syns = process.extract(text, synonym_to_id.keys(), limit=10)#, scorer=fuzz.token_sort_ratio)
    ...
    for d in drugs:
        #print(d)
        match = {"cid": name_to_id.get(d[0], 'NIL'),
                 "cname": d[0],
                 "match_score": d[1]/100}
...
def map_to_chebi_api(text):
    ...
    if text in chebi_cache:
        ...
        matches = []
        payload = {"search": text, "searchCategory": "CHEBI+NAME",
                   "maximumResults": 20, "starsCategory": "ALL"}
        r = requests.get(chebi_api_base, params=payload)
        results = ET.fromstring(r.text)
        for r in results.findall(".//a:ListElement", ns):
            #print(r)
            match = {"cid": r.find("a:chebiId", ns).text,
