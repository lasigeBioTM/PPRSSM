...
def get_chebi_patents(corpus_dir, min_match_score, mapto):
    ...
    label_not_found = 0
    #if mapto == "chebi":
    is_a_graph, name_to_id, synonym_to_id, id_to_name, id_to_index = load_chebi("chebi.obo")
    ...
    for idoc, file in enumerate(docs_list):
        ...
        for s in root.iter("snippet"):
            ...
            for ne in s.findall("ne"):
                ...
                if not ne.get("chebi-id") or ne.get("chebi-id").startswith("WO") or "," in ne.get("chebi-id"):
                    chebi_id = "NIL"
                    ref_nils += 1
                    continue
                else:
                ...
                this_entity = entity_string.format(ne.text, normalized_text, ne.get("type"),
                                                   snum, file_id, chebi_id)
                ...
                entity_perfect_matches_incorrect = generate_candidates_for_entity(normalized_text, chebi_id,
                                                                                  mapto, name_to_id,
                                                                                  synonym_to_id,
                                                                                  min_match_score)
    ...
    if total_entities - no_solution > 0:
        ...
        print("baseline accuracy", total_unique_entities_match / (total_unique_entities - no_solution))
