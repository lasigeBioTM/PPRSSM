...
def load_GO(ontology_name):
    ...
    if ontology_name[:2] == "GO":
        ontology_path = "ontologies/GO/go-basic.obo"
        ...
        if node[1]["namespace"] == extended_ontology_name:
            ...
            if 'is_a' in node[1].keys(): #The root node of the ontology does not have is_a relationships
                for related_node in node[1]['is_a']: 
                    ...
                    edge_list.append(relationship) # Build the edge_list with only is_a relationships
    ...
    assert nx.is_directed_acyclic_graph(is_graph), "The loaded ontology is not a directed acyclic graph"
...
if os.path.exists(distance_cache_file):
    ...
    logging.info("loaded distances dictionary with %s entries", str(len(distance_cache)))
...
def no_path_handler(is_graph, node_1, node_2, ontology_name):
    ...
    try: 
        ...
        distance_to_root_1 = nx.shortest_path_length(is_graph, source=node_1, target=root_id)
        distance_to_root_2 = nx.shortest_path_length(is_graph, source=node_2, target=root_id)
...
def get_ontology_distance(is_graph, node_1, node_2, ontology_name):
    ...
    if key1 in distance_cache.keys():
        ...
        try:
            ...
            distance =  path_traversing_root_handler(is_graph, node_1, node_2, ontology_name)
