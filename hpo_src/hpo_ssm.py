...
ssm.semantic_base("hp.db")  # , to_memory=False)
...
def load_hpo(path="http://purl.obolibrary.org/obo/hp.obo"):
    ...
    # print(synonym_to_id)
    print(
        "done.", "# of concepts", len(name_to_id), "# of synonyms", len(synonym_to_id)
    )
...
def map_to_hpo(text, name_to_id, synonym_to_id):
    ...
    if text in hpo_cache:
        ...
        terms = process.extract(
            text.lower(), name_to_id.keys(), scorer=fuzz.token_sort_ratio, limit=10
        )
        # print("best names of ", text, ":", drugs)
        if terms[0][1] == 100:
            terms = [terms[0]]
        if terms[0][1] < 70:
            term_syns = process.extract(
                text.lower(),
                synonym_to_id.keys(),
                limit=10,
    ...
    for t in terms:
        # print(d)
        match = {
            "cid": name_to_id.get(t[0], synonym_to_id.get(t[0], ["NIL"])[0]),
            "cname": t[0],
            "match_score": t[1] / 100,
        }
