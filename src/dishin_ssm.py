...
from chebi_ssm import load_chebi
chebi_graph, name_to_id, synonym_to_id, id_to_name, id_to_index = load_chebi("chebi.obo")
chebi_graph.to_undirected()
from hpo_ssm import load_hpo
hpo_graph, name_to_id, synonym_to_id, id_to_name, id_to_index, alt_id_to_id = load_hpo("hp.obo")
...
def get_ssm(cid1, cid2, measure="resnik_mica"):
    ...
    r = requests.get("http://127.0.0.1:5000/dishin/", params={"entry1": cid1.replace(":", "_"),
...
def call_dishin_api(ontology, entry1, entry2, measure):
    ...
    args = {"ontology": ontology, "entry1": entry1, "entry2": entry2, "measure": measure}
    r = requests.get(base_url, params=args)
    return r.text


def get_dist_ssm(cid1, cid2, ontology="chebi.db", max_dist=100):
    e1 = cid1.replace(":", "_")
    e2 = cid2.replace(":", "_")
    sim = call_dishin_api(ontology, e1, e2, "jc_mica")
    dist = 1/float(sim.split("\t")[-1].strip())
    return dist
...
def get_dist(cid1, cid2, ontology="chebi.db", max_dist=100):
    ...
    if (cid1, cid2, "v4") in ssm_cache:
        logging.debug("dist cached {} {} {} {}".format(cid1, cid2, ssm_cache[(cid1, cid2, "v4")], len(ssm_cache)))
        return ssm_cache[(cid1, cid2, "v4")]
    elif (cid2, cid1, "v4")  in ssm_cache:
        logging.debug("dist cached {} {} {} {}".format(cid1, cid2, ssm_cache[(cid2, cid1, "v4")], len(ssm_cache)))
...
def path_traversing_root_handler(is_graph, node_1, node_2, ontology_name):
    ...
    #    distance = no_path_handler(is_graph, node_1, node_2, ontology_name)
        #distance = -1
        distance = nx.shortest_path_length(is_graph, source=node_1, target="CHEBI:00000")
        distance += nx.shortest_path_length(is_graph, source=node_2, target="CHEBI:00000")
