from dishin_ssm import get_dist, get_ssm, get_n_ancestors, get_n_descendants, get_dist_network
from chebi_ssm import load_chebi, get_best_chebi_id, map_to_chebi, map_to_chebi_api, map_to_dbpedia
from hpo_ssm import map_to_hpo
    ...
    for e in entity_list:
        ...
        for ic, c in enumerate(entity_list[e][:]):
            ...
            candidates_file.write(candidate_string.format(c["id"], c["incount"],
                                                          c["outcount"], c["links"],
                                                          c["url"], c["name"], "predictedType:GPE"))
    if len(entity_list) > 0:
        print("top candidates that link to other entities", first_candidate_link,
...
def update_entity_list(entity_list, solution_found, normalized_text, cid, solution_label_matches_entity):
    updated_list = []
...
def generate_candidates_for_entity(text, cid, mapto, name_to_id, synonym_to_id, min_match_score=0):
    ...
    for i, candidate_match in enumerate(candidate_names):
        ...
        if candidate_match["match_score"] > min_match_score:
            ...
            # first candidate should be solution
            candidate_list.append({"url": candidate_match["cid"], "name": name,
            ...
            if candidate_match["cname"].lower() == text and cid != candidate_match["cid"]:
            ...
            if cid == candidate_match["cid"]:
                ...
                if candidate_match["cname"].lower() == text: # or candidate_match["chebi_name"].lower() == normalized_text[:-1]:
    ...
    if solution_found > -1:
        ...
        candidate_list = update_entity_list(candidate_list, solution_found,
                                                      text, cid, solution_label_matches_entity)
        ...
        if candidate_list:
            ...
            candidate_list = []
    return candidate_list, solution_found == 0, solution_label_matches_entity, entity_has_incorrect_perfect_match
